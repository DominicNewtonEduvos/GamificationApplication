...
//    }
    ...
    public static void loadDataToDB(){
        ...
        if (GetUsernameTemp.haveuid){
            for (int i = 1; i <= TsAndDs.size(); i++){
//                String documentID = String.format("%02d", i);
                newDocRef = db.collection("study_materials").document();
                Map<String, Object> map = new HashMap<>();
                map.put("userID", GetUsernameTemp.getuid());
                map.put("Term", TsAndDs.get(i-1).term);
                map.put("Definition", TsAndDs.get(i-1).definition);
                map.put("Course", AddAndViewInformation.getCourseName());
                newDocRef.set(map)
...
//                        });
            ...
            GetUsernameTemp.loaduid(new GetUsernameTemp.FirestoreuidCallback() {
                @Override
                public void onCallback(String username) {
                    Log.i("Username Retrieved",username);
                }

                @Override
                public void onError(Exception e) {
                    Log.e("Username Not Retrieved", e.toString());
                }
            });
        }
